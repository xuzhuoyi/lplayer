...
SettingsDialog::~SettingsDialog()
{
    //open file
#ifdef Q_OS_WIN
    QSettings settings("HKEY_CURRENT_USER\\Software\\lplayer", QSettings::NativeFormat);
#else
    QSettings settings(QDir::homePath() + "/.config/lplayer.ini", QSettings::IniFormat);
#endif
    ...
    if (toggled)
    {
        ...
        if (!dir.exists("ffmpeg"))
        {
            QMessageBox::warning(this, "Error", tr("FFMPEG is not installed. Please download it from") +
                                ...
                                tr("and place file \"ffmpeg\" into ~/.lplayer/"));
