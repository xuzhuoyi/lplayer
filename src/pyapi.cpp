...
GetUrl::GetUrl(QObject *parent) : QObject(parent)
{
    ...
    exc_GetUrlError = PyErr_NewException("lplayer.GetUrlError", NULL, NULL);
}
...
void GetUrl::start(const char *url, PyObject *callback, PyObject *_data)
{
    ...
    request.setRawHeader("User-Agent", "lplayer");
    reply = access_manager->get(request);
    connect(reply, SIGNAL(finished()), this, SLOT(onFinished()));
    timer->start(10000);
    // Set lplayer.final_url in Python
...
void initAPI()
{
    ...
    apiModule = Py_InitModule("lplayer", methods);
